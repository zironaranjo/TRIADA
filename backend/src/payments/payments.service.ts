...
export class PaymentsService {
    ...
    async handleEvent(event: Stripe.Event) {
        switch (event.type) {
            case 'payment_intent.succeeded':
                ...
                this.logger.log(`💰 Payment succeeded detected: ${paymentIntent.id} - Amount: ${paymentIntent.amount}`);
    ...
    private async handlePaymentSuccess(paymentIntent: Stripe.PaymentIntent) {
        ...
        if (!bookingId) {
            this.logger.warn(`Payment ${paymentIntent.id} missing keys: 'bookingId' in metadata`);
            return;
        }
        ...
        await this.accountingService.generateSettlement(bookingId, paymentIntent.amount / 100);
